...
pub fn time_24(buf: [:0]u8) !usize {
    var now: c.time_t = c.time(0);
    var now_tm: *c.tm = c.localtime(&now);

    return (try std.fmt.bufPrintZ(buf, " {d:0>2}:{d:0>2} ", .{ @intCast(c_uint, now_tm.*.tm_hour), @intCast(c_uint, now_tm.*.tm_min) })).len;
}

pub fn time_12(buf: [:0]u8) !usize {
    var now: c.time_t = c.time(0);
    var now_tm: *c.tm = c.localtime(&now);
